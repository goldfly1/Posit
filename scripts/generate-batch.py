...
def run_batch_pattern(pattern_name: str, batch_size: int, retries: int, use_wiki: bool = True):
    """Generate all variants for a pattern in ONE model call, then Z3 sweep."""
    combos = get_param_combos(pattern_name, batch_size)
    if not combos:
        print(f"[skip] No parameter grid for {pattern_name}")
        return {"passed": 0, "failed": 0, "tokens": 0}
    ...
    # Build the batch prompt — all variants in ONE call
    variant_descriptions = []
    for i, params in enumerate(combos):
        param_str = ", ".join(f"{k}={v}" for k, v in params.items())
        variant_descriptions.append(f"Variant {i:03d}: {param_str}")
    
    variants_block = "\n".join(variant_descriptions)
    
    prompt = f"""You are a Dafny code generator. Generate {len(combos)} VARIANTS of the {pattern_name} pattern.
...
"""
    ...
    # ─── Step 1: Generate all variants in one call ─────────────────────
    print(f"\n[1] Generating {len(combos)} variants in one model call...")
    text, inp, out, elapsed = call_flash(prompt, system, timeout=600)
    total_tokens = inp + out
    print(f"    Done: {elapsed:.1f}s, {inp}+{out} tokens")
    
    # ─── Step 2: Extract all .dfy files from response ──────────────────
    dafny_files = extract_dafny_files(text)
    print(f"[2] Extracted {len(dafny_files)} .dfy files from response")
    
    if len(dafny_files) < len(combos):
        print(f"    WARNING: expected {len(combos)}, got {len(dafny_files)}")
    
    # ─── Step 3: Z3 sweep — verify all at once ─────────────────────────
    print(f"[3] Z3 sweep — verifying {len(dafny_files)} files...")
    results = {"passed": [], "failed": []}
    
    for i, dafny_source in enumerate(dafny_files):
        ...
        if verified:
            print(f"    {variant_name}: ✅ PASS")
            results["passed"].append({"name": variant_name, "index": i})
        else:
            print(f"    {variant_name}: ❌ FAIL ({len(error_lines)} errors)")
            results["failed"].append({
                "name": variant_name, "index": i,
                "source": dafny_source,
                "errors": '\n'.join(error_lines[:10])
            })
...
"""
    ...
    print(f"  Model calls: {1 + retries if results['failed'] else 1}")
    print(f"{'='*60}\n")
