...
def gen_substitution_variant(pattern_name: str, params: dict, index: int) -> str:
    ...
    1. {{placeholder}} — replaced with param value (if pattern has {{key}} markers)
    2. String replacement — replace default values with param values (e.g., ToUpper → ToLower)
    """
    pattern_file = PATTERNS_DIR / f"{pattern_name}.dfy"
    source = pattern_file.read_text()
    
    # Mode 1: {{placeholder}} substitution
    for key, val in params.items():
        placeholder = "{{" + key + "}}"
        source = source.replace(placeholder, str(val))
    
    # Mode 2: String replacement for known pattern parameters
    # Each pattern has a default value in the code; replace it with the grid value
    if pattern_name == "transformer" and "operation" in params:
        source = source.replace("ToUpper(input[i])", f"{params['operation']}(input[i])")
    elif pattern_name == "parser" and "delimiter" in params:
        # Replace delimiter character in parse logic
        delim = params.get("delimiter", ",")
        source = source.replace('","', f"'{delim}'")
    elif pattern_name == "validator" and "rule" in params:
        # Replace validation rule
        source = source.replace("nonEmpty", params.get("rule", "nonEmpty"))
    elif pattern_name == "repository" and "entityType" in params:
        source = source.replace("Entity", params.get("entityType", "Entity"))
    elif pattern_name == "state-machine" and "states" in params:
        source = source.replace("Idle", params.get("states", "Idle").split(",")[0])
    # Graph: traversal type (BFS/DFS/Dijkstra) changes the algorithm too fundamentally
    # for string replacement. Substitution produces the base DFS pattern (1 unique variant).
    # Model generation needed for genuine BFS/Dijkstra variants.
...
def run_batch_pattern(pattern_name: str, batch_size: int, retries: int, use_wiki: bool = True, round_num: int = 0):
    ...
    # Simple prompt — proven to work at 20 variants, 17.6K tokens, 100% pass.
    # The progression prompt was too verbose and caused 65K output token overflow.
...
Generate these variants (each is a small step from the last — change one parameter or add one feature per variant):
{variants_block}
...
10. BE CONCISE — no comments, no explanations. Only Dafny code. 30-60 lines per variant.
"""
