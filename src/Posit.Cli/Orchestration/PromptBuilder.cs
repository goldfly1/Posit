...
public static class PromptBuilder
{
    ...
    private static string BuildArchitecturePrompt(PatternRegistry? registry)
    {
        ...
        sb.AppendLine("    - outputFormat: print template for formatted output. When the spec's expected");
        sb.AppendLine("      stdout is NOT the raw return value (e.g. spec says 'ERROR: 2' but method");
        sb.AppendLine("      returns int 2), set outputFormat to 'PREFIX: {value}' where {value} is");
        sb.AppendLine("      replaced by the computed result. Use the ACTUAL prefix from the spec's");
        sb.AppendLine("      expected output — NOT a placeholder like 'LEVEL'. If the spec says");
        sb.AppendLine("      'ERROR: 2', outputFormat is 'ERROR: {value}'. For multi-line output");
        sb.AppendLine("      like 'Valid: N\\nInvalid: M', the method should return the FULL formatted");
        sb.AppendLine("      string directly and outputFormat should be null — do NOT return a record");
        sb.AppendLine("      type and rely on JSON serialization.");
        sb.AppendLine("    - emptyOutputText: text to print when the computed result is empty,");
        sb.AppendLine("      e.g. 'No entries' for an empty log file (prints INSTEAD of nothing, exit 0).");
        ...
        sb.AppendLine("  \"testCases\": [{\"id\":\"...\",\"name\":\"...\",\"targetType\":\"...\",\"description\":\"...\",\"expectedBehavior\":\"...\",\"input\":\"...\",\"expectedOutput\":\"...\",\"expectedExitCode\":0,\"cliArgs\":\"\",\"outputFormat\":null,\"emptyOutputText\":null}] }],");
