datatype Result<T> = Success(value: T) | Failure(error: string)

datatype XmlSrc = XmlSrc(raw: string)
datatype ObjTarget = ObjTarget(fields: seq<string>)

function CountChar(s: string, c: char): int
  ensures CountChar(s, c) >= 0
  decreases |s|
{
  if |s| == 0 then 0
  else if s[0] == c then 1 + CountChar(s[1..], c)
  else CountChar(s[1..], c)
}

function RepeatStr(n: int, s: string): seq<string>
  requires n >= 0
  ensures |RepeatStr(n, s)| == n
  decreases n
{
  if n == 0 then [] else [s] + RepeatStr(n - 1, s)
}

class XmlToObjectAdapter {
  method Parse(src: XmlSrc) returns (r: Result<ObjTarget>)
    requires |src.raw| > 0
    ensures r.Success? ==> |r.value.fields| >= 1
    ensures r.Failure? ==> r.error == "invalid xml"
  {
    if src.raw[0] != '<' {
      r := Failure("invalid xml");
    } else {
      var n := CountChar(src.raw, '<');
      r := Success(ObjTarget(RepeatStr(n + 1, "")));
    }
  }

  method ParseBatch(srcs: seq<XmlSrc>) returns (r: Result<seq<ObjTarget>>)
    requires forall i :: 0 <= i < |srcs| ==> |srcs[i].raw| > 0
    ensures r.Success? ==> |r.value| == |srcs|
    ensures r.Failure? ==> r.error == "batch failed"
  {
    r := Failure("batch failed");
    var ts: seq<ObjTarget> := [];
    var i := 0;
    while i < |srcs|
      invariant 0 <= i <= |srcs|
      invariant |ts| == i
      invariant r.Failure? ==> r.error == "batch failed"
      decreases |srcs| - i
    {
      var pr := Parse(srcs[i]);
      if pr.Failure? {
        return;
      }
      ts := ts + [pr.value];
      i := i + 1;
    }
    r := Success(ts);
  }

  method ParseWithRetry(src: XmlSrc, maxAttempts: int) returns (r: Result<ObjTarget>)
    requires |src.raw| > 0
    requires maxAttempts > 0
    ensures r.Failure? ==> r.error == "invalid xml"
    decreases maxAttempts
  {
    var attempts := 0;
    r := Failure("invalid xml");
    while attempts < maxAttempts
      invariant 0 <= attempts <= maxAttempts
      invariant r.Failure? ==> r.error == "invalid xml"
      decreases maxAttempts - attempts
    {
      var pr := Parse(src);
      if pr.Success? {
        r := pr;
        return;
      }
      attempts := attempts + 1;
    }
  }
}