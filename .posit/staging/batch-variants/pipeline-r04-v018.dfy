...
datatype LogEntry = LogEntry(id: int, msg: string, seqNo: int)
...
method AuditLog(entity: Entity, seqNum: int) returns (entry: LogEntry)
  ensures entry.id == entity.id
  ensures entry.seqNo == seqNum
{ entry := LogEntry(entity.id, entity.data, seqNum); }
...
method HandleRequest(input: string, token: string, entities: seq<Entity>, nextId: int, seqNum: int)
  returns (logResult: Result<LogEntry>, storeResult: Result<seq<Entity>>)
  requires |input| > 0
  requires NoDupIds(entities)
  ensures logResult.Success? ==> logResult.value.id == nextId
  ensures logResult.Success? ==> logResult.value.seqNo == seqNum
  ensures storeResult.Success? ==> NoDupIds(storeResult.value)
  ensures storeResult.Success? ==> |storeResult.value| == |entities| + 1
{
  var ok := Authenticate(token);
  if !ok {
    logResult := Failure("auth failed");
    storeResult := Failure("auth failed");
    return;
  }
  var fields := ParseInput(input);
  var vr := ValidateFields(fields);
  if vr.Invalid? {
    logResult := Failure(vr.reason);
    storeResult := Failure(vr.reason);
    return;
  }
  var entity := TransformToEntity(fields, nextId);
  var entry := AuditLog(entity, seqNum);
  logResult := Success(entry);
  storeResult := StoreEntity(entities, entity);
}