...
datatype Subscription = Sub(subscriberId: int, eventName: string, active: bool)
...
function DefaultEvent(): string { "stateChange" }
  ...
  decreases |subs|
{
  ...
  else { result := Success(subs + [Sub(subscriberId, eventName, true)]); }
}
  ...
  decreases |subs|
{
  ...
  if found {
    var s := subs[idx];
    result := Success(subs[..idx] + [Sub(s.subscriberId, s.eventName, active)] + subs[idx+1..]);
