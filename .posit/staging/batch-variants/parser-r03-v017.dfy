...
method ParseLines(input: string, d: string, q: string, e: string, c: string) returns (r: seq<seq<string>>)
  requires |input| > 0
  requires |d| == 1
  requires |q| == 1
  requires |e| <= 1
  ...
  decreases |input|
{
    ...
    decreases |input| - i
  {
    if input[i] == q[0] {
      ...
      if |cl| > 0 {
        ...
        if !first && !isCom { var f := ParseLine(cl, d, q, e); r := r + [f]; }
        first := false;
        cl := "";
      }
    } else if input[i] != '\r' || inq {
      cl := cl + [input[i]];
    }
    i := i + 1;
  }
  if |cl| > 0 && !first {
    var isCom := |c| == 1 && cl[0] == c[0];
    if !isCom { var f := ParseLine(cl, d, q, e); r := r + [f]; }
  }
}