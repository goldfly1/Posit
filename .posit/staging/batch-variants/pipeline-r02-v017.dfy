...
method CheckAuth(token: string, expected: string) returns (ok: bool)
  requires |expected| > 0
  ensures ok ==> |token| > 0
{
  if |token| == 0 {
  ...
  ok := true;
  var i := 0;
  while i < |token| && i < |expected|
    invariant 0 <= i <= |token|
    invariant 0 <= i <= |expected|
    decreases |token| - i
  {
...
method HandleRequest(input: string, d: string, minF: int, maxF: int, entities: seq<Entity>, nextId: int, expectedToken: string) returns (result: Result<seq<Entity>>)
  requires |input| > 0
  requires |d| == 1
  requires minF >= 3
  requires maxF >= minF
  requires |expectedToken| > 0
  requires NoDupIds(entities)
  ensures result.Success? ==> NoDupIds(result.value)
{
  ...
  assert |fields| >= 3;
  var authOk := CheckAuth(fields[2], expectedToken);
  if !authOk {
