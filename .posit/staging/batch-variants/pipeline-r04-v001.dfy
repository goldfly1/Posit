...
method ValidateAndTransform(fields: seq<string>, nextId: int) returns (result: Result<Entity>)
  requires |fields| >= 1
  ensures result.Success? ==> result.value.id == nextId
  ensures result.Failure? ==> |result.error| > 0
{
  if |fields[0]| == 0 { result := Failure("empty field"); }
  else { result := Success(Record(nextId, fields[0])); }
}
  ...
  ensures result.Success? ==> NoDupIds(result.value)
{
  ...
  var vt := ValidateAndTransform(fields, nextId);
  if vt.Failure? { result := Failure(vt.error); return; }
  result := StoreEntity(entities, vt.value);
}