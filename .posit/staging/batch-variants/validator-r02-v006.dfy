...
method Validate(input: string) returns (result: ValidationResult)
  ...
  ensures result.Valid? ==> 0 < |input| < 50
  ensures result.Invalid? ==> |result.errors| > 0
  ensures result.ValidWithWarnings? ==> |result.warnings| > 0 && |input| == 50
{
  ...
  if |input| > 50 {
    errors := errors + ["input too long: expected at most " + IntToString(50) + " chars"];
  } else if |input| == 50 {
