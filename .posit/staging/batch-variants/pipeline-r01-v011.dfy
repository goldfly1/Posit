...
datatype Ctx = Ctx(input: string, fields: seq<string>, data: string)
...
{
  var f := Split(c.input, '|');
  r := Success(Ctx(c.input, f, c.data));
}
...
method Store(c: Ctx) returns (r: Result<Ctx>)
  requires |c.fields| >= 2
  ensures r.Success? ==> |r.value.fields| >= 2 && r.value.data == c.data
{
  r := Success(c);
}
...
method Run(input: string) returns (r: Result<Ctx>)
  requires |input| > 0
  ensures r.Success? ==> |r.value.fields| >= 2 && r.value.data == r.value.fields[0]
{
  var c := Ctx(input, [], "");
  var p := Parse(c);
  if p.Failure? {
