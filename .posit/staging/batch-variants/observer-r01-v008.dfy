datatype Event = StateChange
datatype Subscription = Sub(id: int, event: Event)
...
method Publish(subs: seq<Subscription>, e: Event, filterId: int) returns (delivered: int)
  ...
  ensures 0 <= delivered <= |subs|
{
  ...
  while i < |subs|
    ...
    decreases |subs| - i
  {
    if subs[i].event == e && (filterId < 0 || subs[i].id == filterId) {
...
method Subscribe(subs: seq<Subscription>, id: int, e: Event) returns (r: Result<seq<Subscription>>)
  requires NoDupSubs(subs)
