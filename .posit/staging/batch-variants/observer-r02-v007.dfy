...
method PublishMessage(subs: seq<Subscription>, eventName: string) returns (delivered: int)
...
method UnsubscribeMessage(subs: seq<Subscription>, sid: int, en: string) returns (r: Result<seq<Subscription>>)
  requires NoDupSubs(subs)
  ensures r.Success? ==> |r.value| == |subs| - 1
  ensures r.Success? ==> NoDupSubs(r.value)
