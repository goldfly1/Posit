// Variant 010: Dijkstra directed (parameter directed for edge addition)
include "result.dfy"
...
method AddEdge(adj: seq<seq<Edge>>, from: int, to: int, w: int, directed: bool) returns (newAdj: seq<seq<Edge>>)
  requires 0 <= from < |adj|
  requires 0 <= to < |adj|
  ensures |newAdj| == |adj|
  ensures forall i :: 0 <= i < |adj| && i != from ==> newAdj[i] == adj[i]
  ensures exists e :: e in newAdj[from] && e.to == to && e.weight == w
  ensures !directed ==> exists e :: e in newAdj[to] && e.to == from && e.weight == w
{
  newAdj := adj[..from] + [adj[from] + [Edge(to, w)]] + adj[from + 1..];
  if !directed {
    newAdj := newAdj[..to] + [newAdj[to] + [Edge(from, w)]] + newAdj[to + 1..];
  }
}
