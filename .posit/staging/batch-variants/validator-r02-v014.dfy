...
method Validate(input: string) returns (result: ValidationResult)
  ...
  ensures result.Valid? ==> 0 < |input| < 1000
  ensures result.Invalid? ==> |result.errors| > 0
  ensures result.ValidWithWarnings? ==> |result.warnings| > 0 && |input| == 1000
{
  ...
  if |input| > 1000 {
    errors := errors + ["input too long: expected at most " + IntToString(1000) + " chars"];
  } else if |input| == 1000 {
