...
method Store(cache: map<int, string>, freq: map<int, int>, sizes: map<int, int>,
             key: int, value: string)
  returns (newCache: map<int, string>, newFreq: map<int, int>, newSizes: map<int, int>)
...
method Invalidate(cache: map<int, string>, freq: map<int, int>, sizes: map<int, int>, key: int)
  returns (newCache: map<int, string>, newFreq: map<int, int>, newSizes: map<int, int>)
  ensures key !in newCache
  ensures key !in newFreq
  ensures key !in newSizes
...
method EvictLFU(cache: map<int, string>, freq: map<int, int>, sizes: map<int, int>,
                keys: seq<int>)
  returns (newCache: map<int, string>, newFreq: map<int, int>, newSizes: map<int, int>)
  requires |keys| > 0
  requires forall k :: k in keys <==> k in freq
  ...
  ensures |newFreq| == |freq| - 1
{
  ...
  newSizes := map k | k in sizes && k != evictKey :: sizes[k];
}