...
method Transform(fields: seq<string>, nextId: int, principal: string) returns (e: Entity)
  requires |fields| >= 1 && |fields[0]| > 0 && |principal| > 0
  ensures e.id == nextId
  decreases |fields|
...
{
  log := [];
  var ok, principal := Auth(token);
  if !ok { log := log + ["auth failed"]; r := Failure("auth failed"); return; }
  log := log + ["auth ok:" + principal];
  var f := Parse(input, d);
  log := log + ["parsed"];
  var v := Validate(f, maxLen);
  if v.Invalid? { log := log + ["validation failed"]; r := Failure(v.msg); return; }
  log := log + ["validation ok"];
  var e := Transform(f, nextId, principal);
  log := log + ["transformed"];
