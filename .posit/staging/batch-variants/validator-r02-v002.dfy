...
method Validate(input: string) returns (result: ValidationResult)
  ...
  ensures result.Valid? ==> 0 < |input| < 10
  ensures result.Invalid? ==> |result.errors| > 0
  ensures result.ValidWithWarnings? ==> |result.warnings| > 0 && |input| == 10
{
  ...
  if |input| > 10 {
    errors := errors + ["input too long: expected at most " + IntToString(10) + " chars"];
  } else if |input| == 10 {
