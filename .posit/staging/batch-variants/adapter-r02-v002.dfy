...
class JsonToObjectAdapter {
  ...
  method ParseBatch(srcs: seq<JsonSrc>) returns (r: Result<seq<ObjTarget>>)
  ...
  method ParseWithRetry(src: JsonSrc, maxAttempts: int) returns (r: Result<ObjTarget>)
    requires |src.raw| > 0
    ...
    decreases maxAttempts
  {
    ...
    while attempts < maxAttempts
      ...
      invariant r.Success? ==> |r.value.fields| >= 1
