...
datatype TimerSub = TimerSub(id: int, ev: string)
...
method PublishTimer(subs: seq<TimerSub>, ev: string) returns (d: int)
...
method SubscribeTimer(subs: seq<TimerSub>, id: int, ev: string) returns (r: Result<seq<TimerSub>>)
  requires NoDupTimer(subs)
  ...
  ensures r.Failure? ==> r.error == "dup"
{
  ...
  if f { r := Failure("dup"); } else { r := Success(subs + [TimerSub(id, ev)]); }
}