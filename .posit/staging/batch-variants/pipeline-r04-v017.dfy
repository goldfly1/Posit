...
method AuthenticateWithRole(token: string, role: string) returns (ok: bool)
  ensures ok ==> |token| > 0 && |role| > 0
{ ok := |token| > 0 && |role| > 0; }
...
method LogResult(entity: Entity) returns (entry: LogEntry)
...
method HandleRequest(token: string, role: string, input: string, entities: seq<Entity>, nextId: int) returns (result: Result<seq<Entity>>)
  requires |input| > 0
  requires NoDupIds(entities)
  ensures result.Success? ==> NoDupIds(result.value)
{
  var ok := AuthenticateWithRole(token, role);
  ...
  if result.Success? { var log := LogResult(entity); }
}