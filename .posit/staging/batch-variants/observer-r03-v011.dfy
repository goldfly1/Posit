...
datatype DataUpdateSub = DataUpdateSub(id: int, ev: string)
...
method PublishDataUpdate(subs: seq<DataUpdateSub>, ev: string) returns (d: int)
...
method SubscribeDataUpdate(subs: seq<DataUpdateSub>, id: int, ev: string) returns (r: Result<seq<DataUpdateSub>>)
  requires NoDupDataUpdate(subs)
  ...
  ensures r.Failure? ==> r.error == "dup"
{
  ...
  if f { r := Failure("dup"); } else { r := Success(subs + [DataUpdateSub(id, ev)]); }
}