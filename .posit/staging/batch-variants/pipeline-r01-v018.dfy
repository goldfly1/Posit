...
method Respond(c: Ctx) returns (r: Outcome<Ctx>)
  requires |c.fields| >= 2 && c.authed
  ensures r.Ok? ==> r.value.fields == c.fields && r.value.authed == c.authed && |r.value.log| == |c.log| + 1 && r.value.response == c.data
...
method Run(input: string) returns (r: Outcome<Ctx>)
  requires |input| > 0
  ensures r.Ok? ==> r.value.authed && |r.value.fields| >= 2 && |r.value.log| == 5 && r.value.data == r.value.fields[0] && r.value.response == r.value.data
