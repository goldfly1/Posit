...
method PublishStateChangeFiltered(subs: seq<Subscription>, eventName: string, eventFilter: string) returns (delivered: int)
  ...
  decreases |subs|
{
  ...
  while i < |subs|
    ...
    decreases |subs| - i
  {
    if subs[i].eventName == eventName && (|eventFilter| == 0 || subs[i].eventName == eventFilter) {
