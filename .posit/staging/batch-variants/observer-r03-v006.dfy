...
datatype MessageSub = MessageSub(id: int, ev: string, prio: int)
...
method PublishMessage(subs: seq<MessageSub>, ev: string, filter: string) returns (d: int)
...
method SubscribeMessage(subs: seq<MessageSub>, id: int, ev: string, prio: int) returns (r: Result<seq<MessageSub>>)
  requires NoDupMessage(subs)
  ...
  ensures r.Failure? ==> r.error == "dup"
{
  ...
  if f { r := Failure("dup"); } else { r := Success(subs + [MessageSub(id, ev, prio)]); }
}

method UnsubscribeMessage(subs: seq<MessageSub>, id: int, ev: string) returns (r: Result<seq<MessageSub>>)
  requires NoDupMessage(subs)
  ensures r.Success? ==> |r.value| == |subs| - 1 && NoDupMessage(r.value)
