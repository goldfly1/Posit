...
datatype Ctx = Ctx(input: string, fields: seq<string>, authed: bool, data: string)
...
{
  var f := Split(c.input, '|');
  r := Success(Ctx(c.input, f, false, c.data));
}
...
method Run(input: string) returns (r: Result<Ctx>)
  requires |input| > 0
  ensures r.Success? ==> r.value.authed && |r.value.fields| >= 2 && r.value.data == r.value.fields[0]
{
