...
method ValidateFields(fields: seq<string>, minF: int, maxF: int) returns (ok: bool, msg: string)
  ...
  ensures ok ==> |fields| > 1 ==> |fields[1]| > 0
  ensures ok ==> |fields| > 2 ==> |fields[2]| > 0
  ensures !ok ==> |msg| > 0
{
  if |fields| < minF {
  ...
  } else if |fields| > 1 && |fields[1]| == 0 {
    ok := false; msg := "empty action";
  } else if |fields| > 2 && |fields[2]| == 0 {
    ok := false; msg := "empty token";
  } else {
    ok := true; msg := "";
  }
}
