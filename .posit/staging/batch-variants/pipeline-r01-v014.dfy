datatype ErrorCode = ErrParse | ErrValidate | ErrAuth | ErrStore | ErrTransform
datatype Outcome<T> = Ok(value: T) | Err(code: ErrorCode, msg: string)
datatype Ctx = Ctx(input: string, fields: seq<string>, authed: bool, log: seq<string>, data: string)
...
{
  var f := Split(c.input, '|');
  r := Ok(Ctx(c.input, f, false, c.log + ["parse"], c.data));
}
