...
datatype ClickSub = ClickSub(id: int, ev: string)
...
method SubscribeClick(subs: seq<ClickSub>, id: int, ev: string) returns (r: Result<seq<ClickSub>>)
  requires NoDupClick(subs)
  ...
  ensures r.Failure? ==> r.error == "dup"
{
  ...
  if f { r := Failure("dup"); } else { r := Success(subs + [ClickSub(id, ev)]); }
}