...
predicate ValidHeaders(headers: seq<string>)
{
  forall i :: 0 <= i < |headers| ==> |headers[i]| > 0
}

method AdaptCsvToObject(headers: seq<string>, row: CsvRow) returns (out: seq<ObjectNode>)
  requires ValidHeaders(headers)
...
method ValidateHeaders(headers: seq<string>) returns (ok: bool)
  ensures ok == ValidHeaders(headers)
{
  ok := ValidHeaders(headers);
}

method CountFields(out: seq<ObjectNode>) returns (n: int)
  ensures n == |out|
  ensures n >= 0
{
  n := |out|;
