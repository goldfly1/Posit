...
datatype StateChangeSub = StateChangeSub(id: int, ev: string, prio: int)
...
method PublishStateChange(subs: seq<StateChangeSub>, ev: string, filter: string) returns (d: int)
...
method SubscribeStateChange(subs: seq<StateChangeSub>, id: int, ev: string, prio: int) returns (r: Result<seq<StateChangeSub>>)
  requires NoDupStateChange(subs)
  ...
  ensures r.Failure? ==> r.error == "dup"
{
  ...
  if f { r := Failure("dup"); } else { r := Success(subs + [StateChangeSub(id, ev, prio)]); }
}

method UnsubscribeStateChange(subs: seq<StateChangeSub>, id: int, ev: string) returns (r: Result<seq<StateChangeSub>>)
  requires NoDupStateChange(subs)
  ensures r.Success? ==> |r.value| == |subs| - 1 && NoDupStateChange(r.value)
