...
method Handle(input: string, d: string, token: string, es: seq<Entity>, nextId: int) returns (r: Result<seq<Entity>>)
  requires |input| > 0 && |d| == 1 && NoDup(es)
  ensures r.Success? ==> NoDup(r.value)
  decreases |es|
{
  var authOk := Auth(token);
  if !authOk { r := Failure("auth failed"); return; }
  var f := Parse(input, d);
  var v := Validate(f);
