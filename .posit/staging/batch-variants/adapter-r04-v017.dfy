...
method AdaptXmlToRecordSafe(xml: string) returns (result: Result<TargetRecord>)
  ensures result.Success? ==> result.value == TargetRecord(xml, xml)
  ensures result.Failure? ==> result.error == "invalid xml"
{
  if |xml| > 0 && xml[0] == '<' {
    result := Success(TargetRecord(xml, xml));
  } else {
    result := Failure("invalid xml");
  }
}

method BatchAdaptXmlRecordSafe(requests: seq<string>) returns (results: seq<Result<TargetRecord>>)
  requires |requests| > 0
  ...
  decreases |requests|
{
  ...
  {
    var r : Result<TargetRecord> := AdaptXmlToRecordSafe(requests[i]);
    results := results + [r];
    i := i + 1;
  }
}