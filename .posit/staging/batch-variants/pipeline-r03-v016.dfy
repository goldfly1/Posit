...
method CheckRole(role: int) returns (res: Result<bool>)
  ensures res.Failure? ==> res.error == "forbidden"
{
  if role == 1 { res := Success(true); } else { res := Failure("forbidden"); }
}
...
method RunPipeline(input: string, token: int, role: int, id: int, store: seq<Entity>, logs: seq<string>) returns (res: Result<seq<Entity>>, newLogs: seq<string>)
  requires |input| > 0
  ensures |newLogs| >= |logs|
{
  ...
  var c := CheckRole(role);
  if c.Failure? { res := Failure(c.error); newLogs := logs; return; }
  var e := Transform(v.value, id);
  res := Store(e, store);
  if res.Success? { newLogs := Log(e, logs); } else { newLogs := logs; }
}